...
${info.selectedDisabilities.map(d => `*${DISABILITY_PEDAGOGICAL_GUIDELINES[d]?.name || `HS khuyết tật ${d}`}: [Mục tiêu cụ thể đã giảm tải/điều chỉnh riêng cho dạng này]`).join('\n')}
  ...
  ${nlsFrameworkContext}
    ...
    - CÔNG THỨC TOÁN HỌC & KHOA HỌC (CHUẨN LATEX 100% TƯƠNG THÍCH MATHTYPE & OMML):
      ...
      + 🚨 CHỐNG DÍNH CHỮ CÔNG THỨC TOÁN: Luôn có khoảng cách (dấu cách) giữa công thức toán $...$ và các từ tiếng Việt xung quanh (ví dụ: "Biểu thức $x^2 - 2x$ không phải", "cho $2x^2y$ và $-5x^2y$... là những đơn thức", "đa thức $x^2 + 2x + 1$", "đa thức $A$ và đa thức $B$"). TUYỆT ĐỐI KHÔNG ĐỂ CÔNG THỨC TOÁN SÁT DÍNH VÀO TỪ BÊN CẠNH.
